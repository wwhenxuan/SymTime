...
class TSTEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.layers = nn.ModuleList(
            [TSTEncoderLayer(d_model=d_model, n_heads=n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm,
    ...
    def forward(self,
        ...
        if attn_mask is not None:
            attn_mask = torch.cat([self.cls_mask.expand(batch_size, -1), attn_mask], dim=1)
...
class TSTEncoderLayer(nn.Module):
    ...
    def forward(self,
                src: Tensor,
                attn_mask: Optional[Tensor] = None) -> Union[Tuple[Tensor, Tensor] or Tensor]:
...
class _ScaledDotProductAttention(nn.Module):
    ...
    def forward(self,
        ...
        if attn_mask is not None:  # attn_mask with shape [q_len x seq_len] - only used when q_len == seq_len
            attn_mask = rearrange(attn_mask, 'b i -> b 1 i 1') * rearrange(attn_mask, 'b i -> b 1 1 i')
