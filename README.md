...
SymTime relies on a large-scale series-symbol bimodal dataset generated by [$S^2$Generator](https://github.com/wwhenxuan/S2Generator) during pre-training. You can generate the data required for pre-training by executing the following script:

~~~
bash ./scripts/s2generator.sh
~~~

For the fine-tuning datasets, you can load from [OneDrive](https://drive.google.com/drive/folders/1ACYD_RxQMPZ4DzHDdB3xkRHCkE8FcdH2?usp=sharing) or [BaiduCloud](). Then place the datasets under the folder `./datasets`.
